...
serve(async (req) => {
  ...
  try {
    const { userId, tableId, amount_to_capture_cents } = await req.json().catch(() => ({}));
    ...
    // Capture the PI (full or partial)
    try {
      const params: Record<string, unknown> = {};
      if (typeof amount_to_capture_cents === "number" && amount_to_capture_cents > 0) {
        params.amount_to_capture = amount_to_capture_cents;
      }
      await stripe.paymentIntents.capture(hold.stripe_payment_intent_id, params);
    } catch (e) {
      return fail(`Stripe capture failed: ${e?.message ?? e}`, 500);
    }

    // Mark captured
    const { error: uErr } = await supabase
      .from("collateral_holds")
      .update({ status: "hold_captured" })
      .eq("id", hold.id);
    if (uErr) return fail(`Failed to update hold status: ${uErr.message}`, 500);

    return ok({ success: true });
  } catch (e) {
    console.error("stripe-capture-hold error:", e);
